...
# Paths
binary_csv_dir = "data/interactions/binary_interactions_csv"
fasta_dir = "data/interim/fasta/fasta_sequences_filtered"
output_hdf5_path = "data/interactions/interaction_dataset.hdf5"

# Device for ESM-2 (GPU if available)
...
with h5py.File(output_hdf5_path, "w") as hdf:
    ...
    for csv_file in tqdm(sorted(os.listdir(binary_csv_dir)), desc=" Processing CSVs"):
        if not csv_file.endswith("_binary.csv"):
            continue

        try:
            base = csv_file.replace("_binary.csv", "")
        ...
        data = [(prot_chain_key, sequence)]
        _, _, tokens = batch_converter(data)
        tokens = tokens.to(device)
        with torch.no_grad():
            results = esm_model(tokens, repr_layers=[33], return_contacts=False)
        token_representations = results["representations"][33].cpu()
        esm2_features = np.stack([
            token_representations[0, i].numpy()
