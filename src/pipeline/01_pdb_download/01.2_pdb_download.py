...
pdb_id_file = "/Users/leonorafonso/Documents/Work/MoreiraLab/Project/Files/rcsb_pdb_ids_03_09.txt"
output_dir = "/Users/leonorafonso/Documents/Work/MoreiraLab/Project/Files/cif"
os.makedirs(output_dir, exist_ok=True)

# Load PDB IDs
with open(pdb_id_file, "r") as f:
    all_pdb_ids = [line.strip().lower() for line in f if line.strip()][:1000]

extra_ids = {"1aa6", "1l2q", "7xcn", "3gpo", "6nuj", "3ti4", "4qdv", "5oo3", "5wqx", "8rpo", "8vjp"}

# Adicionar os extras se não estiverem na lista
all_pdb_ids = list(dict.fromkeys(all_pdb_ids + list(extra_ids)))
print(f"\nLoaded {len(all_pdb_ids)} PDB IDs (incluindo extras).")
...
for pdb_id in all_pdb_ids:
    ...
    exp_method = entry_data.get("rcsb_entry_info", {}).get("experimental_method", "N/A")
    prot_count = entry_data["rcsb_entry_info"].get("polymer_entity_count_protein", 0)

    if exp_method == "N/A" or prot_count == 0:
        continue

    valid_protein_pdb_ids.append(pdb_id)
    resolution = entry_data["rcsb_entry_info"].get("resolution_combined", ["N/A"])[0]

    # 2. Download CIF if not already present
    ...
    # 3. Get organism and chain info from PDBe
        ...
        if pdb_id in ebi_data:
            for entity in ebi_data[pdb_id]:
                ...
                for chain in chains:
                    orgs = set()
                    for src in sources:
                        org = src.get("organism_scientific_name", "unknown")
                        orgs.add(org)
                    if not orgs:
                        orgs.add("unknown")
                    chain_to_organism.setdefault(chain, set()).update(orgs)
                    organisms.update(orgs)

    # Flatten sets for saving
    for chain, orgs in chain_to_organism.items():
        chain_to_organism[chain] = "; ".join(sorted(str(o) if o is not None else "unknown" for o in orgs))

    chain_mapping[pdb_id] = chain_to_organism

    organism_summary = "; ".join(sorted(o for o in organisms if o)) if organisms else "Unknown"
    protein_data.append([pdb_id, exp_method, organism_summary, resolution])
...
df_main.to_csv("/Users/leonorafonso/Documents/Work/MoreiraLab/Project/Files/protein_data.csv", index=False)

# Save chain-level mapping
chain_rows = []
for pdb_id, chains in chain_mapping.items():
    for chain, org in chains.items():
        chain_rows.append([pdb_id, chain, org])
pd.DataFrame(chain_rows, columns=["PDB_ID", "Chain_ID", "Organism"]).to_csv("/Users/leonorafonso/Documents/Work/MoreiraLab/Project/Files/chain_organisms.csv", index=False)
