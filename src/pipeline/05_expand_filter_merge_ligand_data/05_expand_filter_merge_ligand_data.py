...
ligands_per_chain_path = "data/interim/ligands_per_chain_filtered.csv"
protein_data_path = "data/interim/protein_data_filtered.xlsx"
organisms_path = "data/interim/chain_organisms_filtered.xlsx"
covalent_links_path="data/interim/covalent_links/covalent_links.csv"
...
#Remove lines from covalent_linking
print(f"Number of lines before removing covalent ligands: {len(df)}")
covalent_links = pd.read_csv(covalent_links_path, dtype=str, na_filter=False)
covalent_links = covalent_links.rename(columns={"PDB_ID": "PDB ID", "Prot_chain": "Chain ID", "Lig_id": "LIGAND_ID"})

df = df[~df[["PDB ID", "Chain ID", "LIGAND_ID"]]
        .apply(tuple, axis=1)
        .isin(covalent_links[["PDB ID", "Chain ID", "LIGAND_ID"]].apply(tuple, axis=1))]

print(f"Number of lines after removing covalent ligands: {len(df)}")
...
final_df.to_excel("data/interim/filtered_df.xlsx", index=False)