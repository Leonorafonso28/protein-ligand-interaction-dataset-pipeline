...
cif_path = "data/raw/ligand_database/components_09_2025.cif"
ligands_csv_path = "data/interim/ligands_per_chain_filtered.csv"
biolip2_path = "data/raw/excluded_ligands_datasets/ligand_list_excluded_BioLiP2.txt"
excluded_txt_path = "data/raw/excluded_ligands_datasets/excluded_ligand_ids.txt"
extra_excluded_path = "data/raw/excluded_ligands_datasets/excluded_ligands_Q_BioLiP.txt"
output_path = "data/raw/excluded_ligands_datasets/full_excluded_ligand_ids.txt"
...
# Rule 4: Same InChIKey, different SMILES → exclude all involved ligands
print("\n[Rule 4] Checking for same InChIKey with different SMILES...")
to_exclude_inchikey_conflict = set()

for inchikey, group in df_incl.groupby("InChIKey"):
    if inchikey and len(set(group["SMILES"])) > 1:
        print(f"  - InChIKey '{inchikey}' has multiple SMILES:")
        for _, row in group.iterrows():
            print(f"      {row['LIGAND_ID']} → {row['SMILES']}")
        to_exclude_inchikey_conflict.update(group["LIGAND_ID"].tolist())

to_exclude_conflicts = (
    to_exclude_smiles_conflict
    .union(to_exclude_duplicates)
    .union(to_exclude_inchi_conflict)
    .union(to_exclude_inchikey_conflict)
)
...
# Save everything
df_final_incl.to_csv("data/interim/ligands_filtered.csv", index=False)
df_excl.to_csv("data/interim/ligands_excluded.csv", index=False)
df_excl.groupby("REASON").size().to_csv("data/interim/excluded_counts_by_reason.csv")
df_conflict_excluded.to_csv("data/interim/ligands_excluded_conflicts.csv", index=False)
